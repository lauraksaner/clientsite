 ...
 * @version 1.9.1
 */
class Qi_Blocks_Premium_Updater {
	private $api_url = '';
	...
	public function __construct( $_api_url, $_plugin_file, $_api_data = null ) {
		...
		$this->failed_request_cache_key = 'qi_blocks_premium_failed_http_' . md5( $this->api_url );
	...
	public function show_update_notification( $file, $plugin ) {
		...
		if ( ! empty( $update_cache->response[ $this->name ]->sections->changelog ) ) {
			$changelog_link = add_query_arg(
				array(
					'qi_blocks_premium_action' => 'view_plugin_changelog',
		...
		/* translators: the plugin name. */
			esc_html__( 'There is a new version of %1$s available.', 'qi-blocks-premium' ),
			esc_html( $plugin['Name'] )
		);

		if ( ! current_user_can( 'update_plugins' ) ) {
			echo ' ';
			esc_html_e( 'Contact your network administrator to install the update.', 'qi-blocks-premium' );
		} elseif ( empty( $update_cache->response[ $this->name ]->package ) && ! empty( $changelog_link ) ) {
			echo ' ';
			printf(
			/* translators: 1. opening anchor tag, do not translate 2. the new plugin version 3. closing anchor tag, do not translate. */
				esc_html__( '%1$sView version %2$s details%3$s.', 'qi-blocks-premium' ),
			...
			echo ' ';
			printf(
				esc_html__( '%1$sView version %2$s details%3$s or %4$supdate now%5$s.', 'qi-blocks-premium' ),
				'<a target="_blank" class="thickbox open-plugin-details-modal" href="' . esc_url( $changelog_link ) . '">',
				esc_html( $update_cache->response[ $this->name ]->new_version ),
				'</a>',
			...
			printf(
				' %1$s%2$s%3$s',
				'<a target="_blank" class="update-link" href="' . esc_url( wp_nonce_url( $update_link, 'upgrade-plugin_' . $file ) ) . '">',
				esc_html__( 'Update now.', 'qi-blocks-premium' ),
				'</a>'
			);
		}
	...
	public function plugins_api_filter( $_data, $_action = '', $_args = null ) {
		...
		$qi_blocks_premium_api_request_transient = $this->get_cached_version_info();

		//If we have no transient-saved value, run the API, set a fresh transient with the API value, and return that value too right now.
		if ( empty( $qi_blocks_premium_api_request_transient ) ) {
			...
			$_data = $qi_blocks_premium_api_request_transient;
		}
	 ...
	 *
	 * @see Qi_Blocks_Premium_Updater::request_recently_failed
	 *
	 * @since 1.9.1
	 */
	...
	public function show_changelog() {
		...
		if ( empty( $_REQUEST['qi_blocks_premium_action'] ) || 'view_plugin_changelog' !== $_REQUEST['qi_blocks_premium_action'] ) {
			return;
		}
		...
		if ( ! current_user_can( 'update_plugins' ) ) {
			wp_die( esc_html__( 'You do not have permission to install plugin updates', 'qi-blocks-premium' ), esc_html__( 'Error', 'qi-blocks-premium' ), array( 'response' => 403 ) );
		}
	...
	private function get_version_from_remote() {
		 ...
		 */
		$api_params = apply_filters( 'qi_blocks_premium_filter_plugin_updater_api_params', $api_params, $this->api_data, $this->plugin_file );
	...
	public function set_version_info_cache( $value = '', $cache_key = '' ) {
		...
		delete_option( 'qi_blocks_premium_api_request_' . md5( serialize( $this->slug . $this->api_data['license'] . $this->beta ) ) );
	}
	...
	private function verify_ssl() {
		return (bool) apply_filters( 'qi_blocks_premium_api_request_verify_ssl', true, $this );
	}
	...
	private function get_cache_key() {
		...
		return 'qi_blocks_premium_' . md5( serialize( $string ) );
	}
}
