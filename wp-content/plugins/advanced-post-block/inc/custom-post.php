...
class APBCustomPost{
	...
	function orderSubMenu( $menu_ord ){
		...
		$arr = array();
		if( apbIsPremium() ){
			$arr[] = $submenu['edit.php?post_type=apb'][5]; // Advanced Posts
			$arr[] = $submenu['edit.php?post_type=apb'][10]; // Add New

			if( APB_HAS_PRO ){
				$arr[] = $submenu['edit.php?post_type=apb'][11]; // Account FS
			}
		}else{
			if( APB_HAS_FREE ){
				$arr[] = $submenu['edit.php?post_type=apb'][11]; // Upgrade (Free)
			}

			if( APB_HAS_PRO ){
				$arr[] = $submenu['edit.php?post_type=apb'][11]; // Account FS
				$arr[] = $submenu['edit.php?post_type=apb'][12]; // Upgrade FS
			}
		}
