...
if ( ! function_exists( 'bt_social_accounts_shortcode' ) ) {
	...
	function bt_social_accounts_shortcode( $pid = 'options' ) {
		...
		if ( function_exists( 'have_rows' ) && have_rows( 'social_accounts', $pid ) ) {
			...
			// $html .= '<li>';
			// 	$html .= '	<a href="/quotes" aria-label="Quote of the Day" rel="noopener noreferrer"> <i class="fa-sharp fa-solid fa-quote-left"></i>';
			// 	$html .= '<span>Quote of the Day</span>';
			// 	$html .= '</a>';
			// 	$html .= '</li>';
			$html .= '</ul>';
		}
...
add_shortcode( 'project_filter', 'filter_shortcode' );
function filter_shortcode() {
	$filter = "";

	$wcatTerms = get_terms('industry', array('hide_empty' => 0, 'parent' =>0)); 
	$filter .= "<select id='project_filter' onchange='location.href=this.value'>";
	$filter .= "<option value='/projects'>Filter by Industry</option>";

	foreach($wcatTerms as $term) {
		$filter .= "<option value='/industry/" . $term->slug . "'>";
		$filter .= $term->name;
		$filter .= "</option>";
	}
	$filter .= "</select>";
