...
if ( ! function_exists( 'bt_widgets_init' ) ) {
	...
	function bt_widgets_init() {
		...
		register_sidebar(
			array(
				'name'          => esc_attr__( 'Single Blog Sidebar Widget Area', 'basetheme' ),
				'id'            => 'single-blog-sidebar-widget-area',
				'description'   => esc_attr__( 'The single blog widget area', 'basetheme' ),
				'before_widget' => '<li id="%1$s" class="widget-container %2$s">',
				'after_widget'  => '</li>',
				'before_title'  => '<h4 class="widget-title">',
				'after_title'   => '</h4>',
...
if ( ! function_exists( 'bt_enqueue' ) ) {
	...
	function bt_enqueue() {
		 ...
		 */
		wp_enqueue_script( 'bt-gsap', 'https://cdnjs.cloudflare.com/ajax/libs/gsap/3.12.0/gsap.min.js', array(), '1.0', false  );
		wp_enqueue_script( 'bt-scroll', 'https://cdnjs.cloudflare.com/ajax/libs/gsap/3.12.0/ScrollTrigger.min.js', array(), '1.0', false  );
...
add_action('init', 'arco_cpts');

add_action( 'init', 'arco_cpttaxonomy' );
function arco_cpttaxonomy() {
	register_taxonomy(
		'project_type',
		'project',
		array(
