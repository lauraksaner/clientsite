...
if ( !is_single() && !is_search() && !is_category() && !is_404() && !is_tax() ) : 
	$navcolor = get_field('heading_background', $post_id);
elseif ( is_tax() ) :
	$navcolor = 'navy';
else :
	$navcolor = 'white';
...
<div id="wrapper">
	...
	<header id="site-header" class="<?php echo $navcolor; ?>">
		<div class="container-wide container-md-none">
			...
			<a href="<?php echo esc_url( home_url( '/' ) ); ?>" title="<?php echo get_bloginfo( 'name', 'display' ); ?>" rel="Home" class="logo-set" aria-label="<?php echo get_bloginfo( 'name', 'display' ); ?> logo">
				<span class="logo"><?php echo bt_load_svg_from_media( $logo['url'] ); ?></span> 
			</a>
			<?php endif; ?>
			<?php
	...
	<main id="site-main" class="<?php echo $navcolor ?>">
